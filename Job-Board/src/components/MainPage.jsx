...
// Import JobCards component
import JobCards from "../components/JobCards";

// Import specific icons from react-icons library
import { MdWorkOutline } from "react-icons/md";
import { BiWorld } from "react-icons/bi";
// Import demo API data
import { demo } from "../demoApiData";

// Define the MainPage component
function MainPage() {
  // Destructure state from useLocation hook to access passed state
  let { state } = useLocation();

  // Determine the current page from state or default to 0
  let currPage = state == null ? 0 : state.pagination;
  // Define state variables for query, full-time filter, location, and pagination
  const [query, setQuery] = useState("");
  const [fullTime, setFullTime] = useState(false);
  const [location, setLocation] = useState("");
  const [pagination, setPagination] = useState(currPage);

  // Define state variables for actual query and location
  const [actualQuery, setActualQuery] = useState("");
  const [actualLocation, setActualLocation] = useState("");

  // Determine the query and location to use for the API request
  let que = actualQuery == "" ? "Full Stack Developer" : actualQuery;
  let loc = actualLocation == "" ? "India" : actualLocation;

  // Define state variables for API data and error
  const [apiData, setApiData] = useState();
  const [error, setError] = useState(null);

  // Handle form submission to update query and location
  const HandleSubmit = (event) => {
    ...
    // Make the API request and update the state with the response data
  ...
  // Handle full-time checkbox change
  ...
  // Use useEffect to make an initial API request when the component mounts
  useEffect(() => {
    const options = {
      method: "GET",
      url: "https://jsearch.p.rapidapi.com/search",
    ...
    // Make the API request and update the state with the response data or error
    axios
      .request(options)
...
// Export the MainPage component as default
